...
<div class="rv-wrap">
    ...
    {{-- ══ FILTER BAR ══ --}}
    <form method="GET" action="{{ route('manager.reviews.index') }}" id="rv-filter-form">
        <div class="rv-filter-bar">
            ...
            @if(request()->hasAny(['search','status','rating']))
                <a href="{{ route('manager.reviews.index') }}" class="rv-bulk-btn outline">
    ...
    {{-- ══ BULK ACTIONS ══ --}}
    <form method="POST" action="{{ route('manager.reviews.bulk') }}" id="rv-bulk-form">
        @csrf
                        ...
                        <tr id="rv-row-{{ $review->id }}">
                            ...
                            <td>
                                <div class="rv-actions">
                                    ...
                                    @else
                                        <form action="{{ route('manager.reviews.unapprove', $review->id) }}" method="POST" style="display:inline;">
                                            @csrf
...
function rvConfirmDelete(id) {
    ...
    form.action = '{{ url("manager/reviews") }}/' + id;
