...
@section('title', 'Pixel Management')
...
<style>
    ...
    .row-pid { font-family: 'DM Mono', 'Courier New', monospace; font-size: 0.85rem; color: #c8cbe0; letter-spacing: 0.5px; }
    ...
    .btn-del { padding: 0.38rem 1rem; background: rgba(226, 75, 74, 0.1); border: 1px solid rgba(226, 75, 74, 0.25); color: #e24b4a; font-size: 0.78rem; font-weight: 500; border-radius: 8px; cursor: pointer; transition: all 0.2s; }
...
<div class="pixel-wrap">
    ...
    <div class="flash-success">
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
        {{ session('success') }}
    </div>
    @endif

    <div class="pixel-header">
        <h1 class="pixel-title">Pixel <span>Manager</span></h1>
        <a href="{{ route('admin.pixels.create') }}" class="btn-add">
    ...
    <div class="card-table">
        ...
        <div class="table-row">
            <span class="row-id">{{ $pixel->id }}</span>
            <span class="row-pid">{{ $pixel->pixel_id }}</span>
            <span>
                @if($pixel->status)
            ...
            <div class="actions">
                <a href="{{ route('admin.pixels.edit', $pixel->id) }}" class="btn-edit">Edit</a>
                <form action="{{ route('admin.pixels.destroy', $pixel->id) }}" method="POST"
        ...
        <div class="empty-state">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            <p>No pixels found. Add your first tracking pixel.</p>
        </div>
        @endforelse
    </div>
