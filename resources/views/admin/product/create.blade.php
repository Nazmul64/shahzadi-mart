...
.section-heading   { font-size:.95rem; font-weight:700; color:var(--brand); margin-bottom:16px; padding-bottom:10px; border-bottom:1px solid #e9ecef; }
...
.btn-create-product   { background:var(--brand); color:#fff; border:none; border-radius:4px; padding:10px 32px; font-size:.95rem; font-weight:600; width:100%; margin-top:10px; }
.btn-create-product:hover { background:var(--brand-dark); color:#fff; }
...
.btn-add-variant-main { background:none; border:1px dashed var(--brand); color:var(--brand); border-radius:4px; padding:8px 16px; font-size:.85rem; cursor:pointer; width:100%; display:flex; align-items:center; justify-content:center; gap:6px; }
...
<div class="d-flex align-items-center gap-3 mb-2">
    <h4 style="font-size:1.1rem; font-weight:700; margin:0; color:var(--brand);">Add Product</h4>
    <a href="{{ route('admin.products.index') }}" class="btn-back">&#8592; Back</a>
</div>
<small class="text-muted" style="font-size:.82rem;">Dashboard &rsaquo; Products &rsaquo; All Products &rsaquo; Add Product</small>
...
<form action="{{ route('admin.products.store') }}" method="POST"
      enctype="multipart/form-data" class="mt-3" id="productForm">
    @csrf
    ...
    <div class="row g-3">
            ...
            <div class="card border-0 shadow-sm p-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="vendor" class="form-control"
                           placeholder="e.g. Test Stores" value="{{ old('vendor') }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="brand_ids[]" class="form-select select2-multi" multiple>
                        @foreach($brands as $brand)
                            <option value="{{ $brand->id }}"
                                {{ in_array($brand->id, old('brand_ids', [])) ? 'selected' : '' }}>
                                {{ $brand->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="color_ids[]" class="form-select select2-multi" multiple>
                ...
                {{-- Unit & Size (Multi-select) --}}
                <div class="row g-2 mb-3">
                    <div class="col-md-6">
                        <label class="form-label-custom">Unit <span class="optional-badge">Optional</span></label>
                        <select name="unit_ids[]" class="form-select select2-multi" multiple>
                    ...
                    <div class="col-md-6">
                        <label class="form-label-custom">Size <span class="optional-badge">Optional</span></label>
                        <select name="size_ids[]" class="form-select select2-multi" multiple>
                ...
                <div class="mb-3">
                    <label class="form-label-custom">
                        ...
                        <a href="#" onclick="generateSku(event)" style="font-size:.8rem; color:var(--brand); margin-left:8px; font-weight:600;">Generate Code</a>
                    </label>
                ...
                <div class="mb-3">
                    ...
                    <div class="seo-section mt-3" id="seo_fields" style="{{ old('allow_seo') ? '' : 'display:none;' }}">
                        ...
                        <div class="mb-0">
                            ...
                            <textarea name="meta_description" class="form-control" rows="4" placeholder="Meta Description">{{ old('meta_description') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                {{-- Description --}}
                <div class="mb-3">
                    <label class="form-label-custom">Product Description <span class="text-danger">*</span></label>
                    <textarea name="description" id="product_description" class="form-control @error('description') is-invalid @enderror" rows="6">{{ old('description') }}</textarea>
        ...
        <div class="col-lg-4">
            ...
            <div class="sidebar-card">
                ...
                @error('feature_image')<small class="text-danger d-block mt-1">{{ $message }}</small>@enderror
            </div>

            {{-- Gallery --}}
            <div class="sidebar-card">
                <div class="sidebar-card-title">Gallery Images <span class="optional-badge">Optional</span></div>
            ...
            {{-- Pricing & Stock --}}
            <div class="sidebar-card">
                <div class="sidebar-card-title">Pricing &amp; Stock</div>
                <div class="mb-3">
                    <label class="form-label-custom">Current Price <span class="text-danger">*</span> <span class="form-label-sub">(BDT)</span></label>
                ...
                <div class="mb-3">
                    <label class="form-label-custom">Discount Price <span class="optional-badge">Optional</span></label>
                    <input type="number" step="0.01" name="discount_price" class="form-control"
                           placeholder="e.g. 450" value="{{ old('discount_price') }}">
                </div>
                <div class="mb-3">
                    <div class="d-flex align-items-center justify-content-between mb-1">
                        <label class="form-label-custom mb-0">Stock Quantity <span class="text-danger">*</span></label>
                        <div class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="is_unlimited" id="is_unlimited"
                                   value="1" onchange="toggleUnlimited(this)" {{ old('is_unlimited') ? 'checked' : '' }}>
                    ...
                    <input type="number" min="0" name="stock" id="stock_input" class="form-control"
                           placeholder="e.g. 100" value="{{ old('stock', 0) }}" {{ old('is_unlimited') ? 'disabled' : '' }}>
                </div>
                <div class="mb-0">
                    <label class="form-label-custom">YouTube URL <span class="optional-badge">Optional</span></label>
            ...
            <div class="flash-sale-card">
                ...
                <div class="toggle-switch-wrap mb-3">
                    <div class="form-check form-switch mb-0">
                        <input class="form-check-input" type="checkbox" name="is_flash_sale" id="is_flash_sale"
                               value="1" onchange="toggleFlashSale(this)" {{ old('is_flash_sale') ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_flash_sale">Enable Flash Sale</label>
                    </div>
                </div>
                <div id="flashSaleFields" style="{{ old('is_flash_sale') ? '' : 'display:none;' }}">
                    <div class="mb-3">
                        <label class="form-label-custom">Flash Sale Price <span class="text-danger">*</span> <span class="form-label-sub">(BDT)</span></label>
                        <input type="number" step="0.01" name="flash_sale_price" class="form-control"
                               placeholder="e.g. 399" value="{{ old('flash_sale_price') }}">
                        <small class="text-muted" style="font-size:.75rem;">Overrides discount price during the sale</small>
                    </div>
                    <div class="mb-3">
            ...
            <div class="new-arrival-card">
                ...
                <small class="text-muted d-block mt-2" style="font-size:.75rem;">Product will appear in the New Arrivals section.</small>
            </div>
...
<script>
var URL_GET_SUBS     = "{{ route('admin.products.getSubCategories') }}";
var URL_GET_CHILDREN = "{{ route('admin.products.getChildCategories') }}";
var galleryItems     = [];
...
$(document).ready(function () {
    ...
    $('#product_category').on('change', function () {
        ...
        if (!catId) { $('#sub_hint').text('Select a category above to load sub categories'); return; }
        $('#sub_hint').html('<span style="color:#e67e22;">Loading...</span>');
        $.get(URL_GET_SUBS, { category_id: catId })
            .done(function(data) {
                var html = '<option value="">-- Select Sub Category --</option>';
                $.each(data, function(i,r) { html += '<option value="'+r.id+'">'+r.sub_name+'</option>'; });
                $('#product_subcategory').html(html);
                $('#sub_hint').html(data.length ? '<span style="color:green;">&#10003; '+data.length+' found</span>' : '<span style="color:#e67e22;">No sub categories found</span>');
            })
            .fail(function(xhr) { $('#sub_hint').html('<span style="color:red;">Error '+xhr.status+'</span>'); });
    });
    ...
    $('#product_subcategory').on('change', function () {
        ...
        $.get(URL_GET_CHILDREN, { sub_category_id: subId })
            .done(function(data) {
                ...
                $('#child_hint').html(data.length ? '<span style="color:green;">&#10003; '+data.length+' found</span>' : '<span style="color:#e67e22;">No child categories found</span>');
            })
            .fail(function(xhr) { $('#child_hint').html('<span style="color:red;">Error '+xhr.status+'</span>'); });
    });
...
function generateSku(e) {
    e.preventDefault();
    var chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', nums = '0123456789', p='', n='', s='';
    ...
    document.getElementById('sku_input').value = p+n+s;
}

function toggleUnlimited(cb) {
    var si = document.getElementById('stock_input');
    si.disabled = cb.checked;
    if(cb.checked) si.value = '';
}

function toggleFlashSale(cb) { cb.checked ? $('#flashSaleFields').slideDown(250) : $('#flashSaleFields').slideUp(250); }

function renderGallery() {
    var preview = $('#galleryPreview'); preview.empty();
    galleryItems.forEach(function(item, idx) {
        var reader = new FileReader();
        reader.onload = function(e) {
            var card = $('<div class="gallery-card" id="gc_'+idx+'"></div>');
            card.append('<img src="'+e.target.result+'">');
            card.append('<input type="text" class="form-control form-control-sm mt-1" placeholder="Size" value="'+(item.size||'')+'" onchange="galleryItems['+idx+'].size=this.value;rebuildGalleryInputs();">');
            card.append('<input type="text" class="form-control form-control-sm mt-1" placeholder="Color" value="'+(item.color||'')+'" onchange="galleryItems['+idx+'].color=this.value;rebuildGalleryInputs();">');
...
function rebuildGalleryInputs() {
    var container = $('#galleryFileInputs'); container.empty();
