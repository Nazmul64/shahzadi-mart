...
<style>
:root {
    --brand: #5b3cf5; --brand-light: #ede9ff; --brand-dark: #3d20d6;
    --surface: #ffffff; --surface-2: #f8f7fc;
    --border: rgba(91,60,245,.10); --border-2: rgba(91,60,245,.20);
    --text-1: #14112a; --text-2: #5c587a; --text-3: #9a96b5;
...
.edit-indicator { display: inline-flex; align-items: center; gap: 8px; background: var(--warning-bg); color: var(--warning); font-size: 12px; font-weight: 600; padding: 5px 12px; border-radius: 20px; margin-bottom: 20px; border: 1px solid #fde68a; }
...
/* Media Type */
.media-type-group { display: flex; gap: 12px; }
...
/* Current image */
.current-image-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 8px 12px; background: var(--surface-2); border-radius: var(--radius-sm); border: 1px solid var(--border); }
.current-image-row img { width: 56px; height: 42px; object-fit: cover; border-radius: 6px; border: 1px solid var(--border-2); }
.current-image-row .img-info { flex: 1; }
.current-image-row .img-info strong { display: block; font-size: 12px; font-weight: 600; color: var(--text-2); }
.current-image-row .img-info span { font-size: 11px; color: var(--text-3); }
.current-badge { background: var(--success-bg); color: var(--success); font-size: 10px; font-weight: 600; padding: 2px 8px; border-radius: 10px; }

.new-preview-box { margin-top: 8px; display: none; align-items: center; gap: 10px; }
.new-preview-box img { width: 56px; height: 42px; object-fit: cover; border-radius: 6px; border: 2px solid var(--brand); }
.new-preview-box span { font-size: 12px; color: var(--brand); font-weight: 500; }

/* Video */
.current-video-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 10px 14px; background: var(--surface-2); border-radius: var(--radius-sm); border: 1px solid var(--border); }
.current-video-icon { width: 42px; height: 42px; background: var(--brand-light); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: var(--brand); font-size: 16px; flex-shrink: 0; }
.current-video-row .img-info { flex: 1; }
...
.btn-update { display: inline-flex; align-items: center; gap: 8px; background: var(--brand); color: #fff; font-size: 14px; font-weight: 600; padding: 11px 28px; border-radius: var(--radius-sm); border: none; cursor: pointer; transition: background .18s, transform .15s, box-shadow .18s; box-shadow: 0 2px 8px rgba(91,60,245,.28); }
.btn-update:hover { background: var(--brand-dark); transform: translateY(-1px); box-shadow: 0 4px 16px rgba(91,60,245,.38); }
...
<div class="container-fluid py-1">
    ...
    <div class="page-header">
        <h4 class="page-title">
            Edit Campaign
            <span>Updating: <strong>{{ $campaign->title }}</strong></span>
        </h4>
        <a href="{{ route('admin.campaigncreate.index') }}" class="btn-manage">
            <i class="fas fa-list-ul"></i> All Campaigns
        </a>
    </div>

    <div class="edit-indicator">
        <i class="fas fa-pencil-alt"></i> You are editing an existing campaign — changes will go live immediately
    </div>
    ...
    <div class="form-card">
        <form action="{{ route('admin.campaigncreate.update', $campaign->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <select name="product_id" class="form-select @error('product_id') is-invalid @enderror">
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}"
                                {{ old('product_id', $campaign->product_id) == $product->id ? 'selected' : '' }}>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            {{-- Media Type --}}
            @php $currentType = old('media_type', $campaign->media_type); @endphp
            <div class="form-section">
                <div class="section-label">Media Type <span class="required-dot">*</span></div>
                <input type="hidden" name="media_type" id="media_type_input" value="{{ $currentType }}">
                <div class="media-type-group">
                    <div class="media-type-btn {{ $currentType === 'Image' ? 'active' : '' }}"
                         onclick="setMediaType('Image', this)">
                    ...
                    <div class="media-type-btn {{ $currentType === 'Video' ? 'active' : '' }}"
                         onclick="setMediaType('Video', this)">
            ...
            <div class="form-section media-fields" id="image_fields"
                 style="{{ $currentType === 'Video' ? 'display:none' : '' }}">
                ...
                <div class="mb-4">
                    ...
                    <div class="current-image-row">
                        <img src="{{ asset($campaign->image) }}" alt="current">
                        <div class="img-info">
                            <strong>{{ basename($campaign->image) }}</strong>
                    ...
                    <div class="file-upload-area" onclick="document.getElementById('image_input').click()">
                        <div class="file-upload-icon"><i class="fas fa-sync-alt"></i></div>
                        <div class="file-upload-text">
                            <strong id="image_name">{{ $campaign->image ? 'Replace image (optional)' : 'Upload image' }}</strong>
                    ...
                    <div class="new-preview-box" id="image_prev">
                        <img src="" alt=""><span><i class="fas fa-arrow-up me-1"></i>New image selected</span>
                    </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div class="current-image-row">
                            <img src="{{ asset($campaign->image_two) }}" alt="">
                            <div class="img-info"><strong>Current</strong><span>Secondary image</span></div>
                            <span class="current-badge">Current</span>
                        </div>
                        @endif
                        <div class="file-upload-area" onclick="document.getElementById('image_two_input').click()">
                            ...
                            <div class="file-upload-text">
                                <strong id="image_two_name">{{ $campaign->image_two ? 'Replace' : 'Upload' }} secondary</strong>
                                <span>Optional</span>
                            </div>
                            <input type="file" id="image_two_input" name="image_two" accept="image/*"
                                   onchange="previewFile(this, 'image_two_name', 'image_two_prev')">
                        </div>
                        <div class="new-preview-box" id="image_two_prev"><img src="" alt=""><span>New image</span></div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Tertiary Image</label>
                        @if($campaign->image_three)
                        <div class="current-image-row">
                            <img src="{{ asset($campaign->image_three) }}" alt="">
                            <div class="img-info"><strong>Current</strong><span>Tertiary image</span></div>
                            <span class="current-badge">Current</span>
                        </div>
                        @endif
                        <div class="file-upload-area" onclick="document.getElementById('image_three_input').click()">
                            ...
                            <div class="file-upload-text">
                                <strong id="image_three_name">{{ $campaign->image_three ? 'Replace' : 'Upload' }} tertiary</strong>
                                <span>Optional</span>
                            </div>
                        ...
                        <div class="new-preview-box" id="image_three_prev"><img src="" alt=""><span>New image</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-section media-fields" id="video_fields"
                ...
                @if($campaign->video)
                <div class="current-video-row">
                    <div class="current-video-icon"><i class="fas fa-film"></i></div>
                    <div class="img-info">
                        <strong>{{ basename($campaign->video) }}</strong>
                        <span>Current uploaded video</span>
                    </div>
                    <span class="current-badge">Current</span>
                </div>
                @endif
                ...
                <div class="mb-3">
                    <label class="form-label">{{ $campaign->video ? 'Replace Video File' : 'Upload Video File' }}</label>
                    <div class="file-upload-area" onclick="document.getElementById('video_input').click()">
                        ...
                        <div class="file-upload-text">
                            <strong id="video_name">{{ $campaign->video ? 'Click to replace video' : 'Click to upload video' }}</strong>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="review"
                               ...
                               value="{{ old('review', $campaign->review) }}"
                               placeholder="e.g. 4.8/5 rating">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="current-image-row mb-2">
                            <img src="{{ asset($campaign->review_image) }}" alt="">
                            <div class="img-info"><strong>Current review image</strong><span>Will keep if unchanged</span></div>
                            <span class="current-badge">Current</span>
                        </div>
                        @endif
                        <div class="file-upload-area" onclick="document.getElementById('review_image_input').click()">
                            ...
                            <div class="file-upload-text">
                                <strong id="review_image_name">{{ $campaign->review_image ? 'Replace review image' : 'Upload review image' }}</strong>
                                <span>Required</span>
                            </div>
                            <input type="file" id="review_image_input" name="review_image" accept="image/*"
                                   onchange="previewFile(this, 'review_image_name', 'review_prev')">
                        </div>
                        <div class="new-preview-box" id="review_prev"><img src="" alt=""><span>New image selected</span></div>
            ...
            <div class="form-section">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="short_description" id="short_description" class="form-control" rows="4">{{ old('short_description', $campaign->short_description) }}</textarea>
                </div>
                <div>
                    <label class="form-label">Full Description</label>
                    <textarea name="description" id="description" class="form-control" rows="5">{{ old('description', $campaign->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="form-footer">
                ...
                <a href="{{ route('admin.campaigncreate.index') }}" class="btn-cancel">
