...
<div class="pdp">
  ...
  <div class="pdp__breadbar" style="background: #fff; border-bottom: 1px solid #eee;">
    <div class="pdp__wrap" style="padding: 15px 40px !important;">
  ...
  <div class="pdp__wrap">
    <div class="pdp__card">
        <div class="pdp__grid">
          <div class="pdp__gallery" style="order: 1;">
          ...
          <div class="pdp__info" style="order: 2;">
            <h1 class="pdp__title">{{ $product->name }}</h1>
            
            <div class="pdp__quick-details mb-3">
                Quick Details ID No: HM{{ $product->id }}-856 Price: {{ number_format($displayPrice, 0) }} BDT Brand: {{ $product->brand_name ?? 'N/A' }} Model: {{ $product->model ?? 'N/A' }} Product Title: {{ $product->name }}
            </div>

            <div class="pdp__quick-meta mb-4">
                <div class="pdp__stars">
                    @for($i=1;$i<=5;$i++)
                        <i class="bi bi-star{{ $i <= round($avgRating) ? '-fill' : '' }} {{ $i <= round($avgRating) ? 'text-warning' : 'text-muted' }}"></i>
                    @endfor
                    <span class="ms-1 text-muted">0 Review</span>
                </div>
                <span class="pdp__sep">|</span>
                <span class="text-muted">1 Sold</span>
                <span class="pdp__sep">|</span>
                <button class="pdp__share-trigger"><i class="bi bi-share"></i> Share</button>
              <button class="pdp__wish-trigger ms-3"><i class="bi bi-heart"></i></button>
              <button class="pdp__chat-trigger ms-3" id="pdpChatTrigger" data-seller-id="{{ $seller ? $seller->id : $product->vendor }}" data-product-id="{{ $product->id }}">
          ...
          <div class="pdp__sidebar" style="order: 3;">
            {{-- Delivery Charge Card --}}
            <div class="pdp__side-card pdp__side-card--delivery mb-3">
              <div class="pdp__del-row">
                <div class="pdp__del-icon"><i class="bi bi-box-seam"></i></div>
                <div class="pdp__del-info">
                  <div class="pdp__del-label">Delivery charge</div>
                  <div class="pdp__del-val">$2.00</div>
                </div>
              </div>
              <div class="pdp__del-row">
                <div class="pdp__del-icon"><i class="bi bi-clock-history"></i></div>
                <div class="pdp__del-info">
                  <div class="pdp__del-label">Estimated delivery</div>
                  <div class="pdp__del-val">1</div>
                </div>
              </div>
            </div>
            ...
            {{-- Sold By Card --}}
            @if($seller)
              <div class="pdp__side-card pdp__side-card--seller mb-3">
                <div class="pdp__sel-top">
                  <div class="pdp__sel-logo">
                  ...
                  <div class="pdp__sel-name-wrap">
                    <div class="pdp__sel-label">Sold by</div>
                    <div class="pdp__sel-name">{{ $seller->store_name }}</div>
                  </div>
                  <div class="pdp__sel-rating">
                    <i class="bi bi-star-fill"></i>
                    <span>{{ number_format($seller->avg_rating ?? 5.0, 1) }}</span>
                  </div>
                </div>
                <div class="text-center mt-2">
                  <a href="{{ route('shop.details', $seller->store_slug) }}" class="pdp__sel-link">Visit Store</a>
                </div>
              </div>

              {{-- Popular Products From Them --}}
              @if($sellerProducts->isNotEmpty())
                <div class="pdp__pop-sec">
                  <h4 class="pdp__pop-title">Popular Products From Them</h4>
                  <div class="pdp__pop-list">
                    @foreach($sellerProducts as $sp)
                      <div class="pdp__pop-item">
                        <div class="pdp__pop-img">
                          <img src="{{ asset('uploads/products/'.$sp->feature_image) }}" alt="{{ $sp->name }}">
                        </div>
                        <div class="pdp__pop-content">
                          <div class="pdp__pop-name">{{ Str::limit($sp->name, 40) }}</div>
                          <div class="pdp__pop-price">৳{{ number_format($sp->discount_price ?? $sp->current_price, 0) }}</div>
                          <div class="pdp__pop-meta">
                            <i class="bi bi-star-fill text-warning"></i> {{ number_format($sp->reviews_avg_rating ?? 5.0, 1) }} (0) 
                            <span class="ms-2">| 6 Sold</span>
                          </div>
                          <a href="{{ route('product.detail', $sp->slug) }}" class="pdp__pop-buy">
                            <i class="bi bi-cart"></i> Buy Now <i class="bi bi-arrow-right-short"></i>
                          </a>
                        </div>
                      </div>
                    @endforeach
...
.pdp-chat-seller-info {
    display: flex;
    align-items: center;
    gap: 10px;
}

.pdp-chat-seller-info img {
    width: 32px;
    height: 32px;
    border-radius: 50%;
    background: #fff;
...
.pdp-chat-msg--me { justify-content: flex-end; }
.pdp-chat-msg--them { justify-content: flex-start; }
...
.pdp-chat-msg--me .pdp-chat-msg-bubble {
    background: var(--primary);
    color: #fff;
    border-bottom-right-radius: 2px;
}

.pdp-chat-msg--them .pdp-chat-msg-bubble {
    background: #e2e8f0;
    color: #1e293b;
...
.pdp__chat-trigger:hover {
    background: #f8f9fa;
    border-color: var(--primary);
    color: var(--primary);
}

/* ── FORCE FULL WIDTH (Override Master) ── */
.main-site-content.container-fluid, 
...
.pdp__grid {
    display: grid;
    grid-template-columns: 650px 2fr 350px;
    gap: 20px;
    align-items: start;
    padding: 0 40px;
}
...
{{-- ════════ CHAT MODAL ════════ --}}
<div class="pdp-chat-box" id="pdpChatBox">
    <div class="pdp-chat-head">
        <div class="pdp-chat-seller-info">
            <img src="{{ $seller->store_logo ? asset($seller->store_logo) : 'https://ui-avatars.com/api/?name='.$seller->store_name }}" alt="">
            <span>Chat with {{ $seller->store_name }}</span>
        </div>
        <button class="pdp-chat-close" id="pdpChatClose"><i class="fas fa-times"></i></button>
    </div>
    <div class="pdp-chat-body" id="pdpChatBody">
        <div class="pdp-chat-placeholder">Start a conversation with the seller...</div>
    </div>
    <div class="pdp-chat-foot">
        <input type="text" id="pdpChatInput" placeholder="Type your message...">
        <button id="pdpChatSend" onclick="alert('Send button clicked!'); sendMessage();"><i class="fas fa-paper-plane"></i></button>
    </div>
</div>
...
<script>
    ...
    (function() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const chatTrigger = document.getElementById('pdpChatTrigger');
            const chatBox     = document.getElementById('pdpChatBox');
            const chatClose   = document.getElementById('pdpChatClose');
            const chatBody    = document.getElementById('pdpChatBody');
            const chatInput   = document.getElementById('pdpChatInput');
            const chatSend    = document.getElementById('pdpChatSend');
            ...
            function loadChatMessages() {
                ...
                const sellerId = chatTrigger.dataset.sellerId;
                fetch(`/chat/messages/${sellerId}`)
                    .then(res => res.json())
                    .then(res => {
            ...
            function renderMessages(data) {
                ...
                const sessionUuid = data.session_uuid;
                ...
                messages.forEach(msg => {
                    ...
                    if (msg.sender_type === 'customer') {
                        if (currentUserId && msg.customer_id == currentUserId) isMe = true;
                        else if (!currentUserId && msg.session_uuid === sessionUuid) isMe = true;
                    }

                    const div = document.createElement('div');
                    div.className = `pdp-chat-msg ${isMe ? 'pdp-chat-msg--me' : 'pdp-chat-msg--them'}`;
                    
                    let avatarHtml = '';
                    if (!isMe) {
                        const avatarUrl = msg.seller ? (msg.seller.store_logo ? `/${msg.seller.store_logo}` : `https://ui-avatars.com/api/?name=${msg.seller.store_name}`) : 'https://ui-avatars.com/api/?name=Seller';
                        avatarHtml = `<img src="${avatarUrl}" class="pdp-chat-avatar" alt="">`;
                    }

                    div.innerHTML = `
                        ${avatarHtml}
                        <div class="pdp-chat-msg-bubble">${msg.message}</div>
                    `;
            ...
            window.pdpSendMessage = function() {
                if(!chatInput || !chatTrigger) return;
                const msg = chatInput.value.trim();
                if (!msg) return;

                const sellerId = chatTrigger.dataset.sellerId;
                const productId = chatTrigger.dataset.productId;
            ...
            if (chatSend) {
                chatSend.addEventListener('click', window.pdpSendMessage);
            }
